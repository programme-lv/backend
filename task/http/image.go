...
func (h *TaskHttpHandler) UploadStatementImage(w http.ResponseWriter, r *http.Request) {
	...
	if !allowedChars.MatchString(uploadedFilename) {
	...
	if len(uploadedFilename) > 100 {
		errMsg := fmt.Sprintf("filename is too long (max 100 characters): %s", uploadedFilename)
		errCode := "filename_too_long"
		httpjson.WriteErrorJson(w, errMsg, http.StatusBadRequest, errCode)
		return
	}
	cantContain := []string{"CON", "PRN", "AUX", "NUL", "COM", "LPT"}
	if len(uploadedFilename) < 4 && slices.Contains(cantContain, uploadedFilename) {
		errMsg := fmt.Sprintf("invalid filename (may contain reserved filenames): %s", uploadedFilename)
		errCode := "invalid_filename"
		httpjson.WriteErrorJson(w, errMsg, http.StatusBadRequest, errCode)
		return
	}
	...
	contentTypeMime, imageMimeType, err := getUploadedFileMIMEs(image, header)
	if err != nil {
	...
	if contentTypeMime != imageMimeType {
		errCode := "content_type_mismatch"
		httpjson.WriteErrorJson(w, "Content-Type header does not match detected MIME type", http.StatusBadRequest, errCode)
		return
	}
