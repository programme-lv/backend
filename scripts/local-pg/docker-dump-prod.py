#! /usr/bin/env python3
"""
This script dumps the production database to a local file using Docker to run pg_dump.
Requires Docker and aws cli to be installed.
...
def execute_docker_pg_dump(host, port, user, db_name, password, sslmode, dump_file='./mydb.dump'):
    """
    Executes the pg_dump command inside a Docker container with the provided parameters.
    ...
    """
    # Get the absolute path of the dump file
    abs_dump_file = os.path.abspath(dump_file)
    dump_dir = os.path.dirname(abs_dump_file)
    dump_filename = os.path.basename(abs_dump_file)
    ...
    cmd = [
        ...
        '-v',             # Verbose mode
        '-f', f"/dump/{dump_filename}",  # Output file inside the container
        db_name
    ]
    
    try:
        print("Starting pg_dump with Docker...")
...
def main():
    ...
    # Step 3: Execute pg_dump via Docker
    execute_docker_pg_dump(
        host=host,
        port=port,
        user=user,
        db_name=db_name,
