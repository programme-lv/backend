...
type UserService interface {
	...
	GetPasswordChangedAt(ctx context.Context, userUUID uuid.UUID) (*time.Time, error)
}
...
func (s *userSrvc) GetPasswordChangedAt(ctx context.Context, userUUID uuid.UUID) (*time.Time, error) {
	var pwdChangedAt *time.Time
	err := s.postgres.QueryRow(ctx, `
		SELECT pwd_changed_at FROM users WHERE uuid = $1
	`, userUUID).Scan(&pwdChangedAt)
	if err != nil {
		return nil, err
	}
	return pwdChangedAt, nil
}
