...
//
// submissions WebSocket server streaming
//
// Command:
...
// StreamSubmissionUpdatesServerStream implements the
// submissions.StreamSubmissionUpdatesServerStream interface.
type StreamSubmissionUpdatesServerStream struct {
	once sync.Once
...
// Send streams instances of "submissions.SubmissionListUpdate" to the
// "streamSubmissionUpdates" endpoint websocket connection.
func (s *StreamSubmissionUpdatesServerStream) Send(v *submissions.SubmissionListUpdate) error {
	var err error
	...
	res := v
	body := NewStreamSubmissionUpdatesResponseBody(res)
	return s.conn.WriteJSON(body)
}

// Close closes the "streamSubmissionUpdates" endpoint websocket connection.
func (s *StreamSubmissionUpdatesServerStream) Close() error {
	var err error
	if s.conn == nil {
		return nil
	}
