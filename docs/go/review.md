...
Common comments from Go code reviews and some extra practices from elsewhere.

Treat this as a supplement to [Effective Go](effective.md) ([live](https://go.dev/doc/effective_go)).

Most of the named sections come from [Go Code Review Comments](https://go.dev/wiki/CodeReviewComments). A few extras (Makefile, default `http.Client`, wrapping with context, globals, project layout) are from later write-ups; see [References](#references).

Where this file conflicts with [../go.md](../go.md), the repo page wins.
...
Configure the editor to run `gofmt` on save so unformatted code is not committed.

## Go vet

Run `go vet` on your code to check suspicious constructs before every commit. Vet reports things the compiler may miss, such as `Printf` calls whose arguments do not align with the format string. It uses heuristics: not every report is a real bug.
...
When checking an error, don't check for a specific text in the error, check its type instead. Don't write:
...
Prefer `errors.Is` / `errors.As` and wrapping with `%w` ([Go 1.13 errors](https://go.dev/blog/go1.13-errors)).
...
3. [Errors are values](https://go.dev/blog/errors-are-values)
...
This rule also applies to `ID` when it is short for "identity", so write `appID` instead of `appId`.
...
See [Effective Go — MixedCaps](https://go.dev/doc/effective_go#mixed-caps). This applies even when it breaks conventions in other languages. For example an unexported constant is `maxLength` not `MaxLength` or `MAX_LENGTH`.
...
All references to names in your package will be done using the package name, so you can omit that name from the identifiers. For example, if you are in package `chubby`, you don't need type `ChubbyFile`, which clients will write as `chubby.ChubbyFile`. Instead, name the type `File`, which clients will write as `chubby.File`. Avoid meaningless package names like `util`, `common`, `misc`, `api`, `types`, and `interfaces`. See [Effective Go — Package names](https://go.dev/doc/effective_go#package-names) and [Package names](https://go.dev/blog/package-names).
...
- Can functions or methods, either concurrently or when called from this method, be mutating the receiver? A value type creates a copy of the receiver when the method is invoked, so outside updates will not be applied to this receiver. If changes must be visible in the original receiver, the receiver must be a pointer.
...
```go
if got != tt.want {
	t.Errorf("Foo(%q) = %d; want %d", tt.in, got, tt.want) // or Fatalf, if the test can't continue
}
```
...
Some projects collect `gofmt`, `go vet`, `go test`, and image builds in a Makefile so everyone types `make all`. This repo does not; see [../go.md](../go.md).

Example from the source write-up:
...
Don't use the default `http.Client` in production. `http.DefaultClient` is convenient (`http.Get(url)` or `http.DefaultClient.Get(url)`) but it is created with Timeout 0, meaning no timeout. If the target hangs, so does the caller.

Define a custom `http.Client` with a timeout and reuse it:
...
The same applies for `http.Server`: set timeouts instead of using the zero-value server:
...
1. [Don't use Go's default HTTP client](https://medium.com/@nate510/don-t-use-go-s-default-http-client-4804cb19f779)
...
Wrapping errors with a custom message provides context as they propagate up the stack. Keep the root error accessible for `errors.Is` / `errors.As`.
...
file, err := os.Open("foo.txt")
if err != nil {
	return fmt.Errorf("open foo.txt: %w", err)
}
```

Do not add `github.com/pkg/errors`. See [Go 1.13 errors](https://go.dev/blog/go1.13-errors).

## Avoid global variables

Global variables make testing and readability hard. They also make everyone in the same package able to access state they should not. Prefer a struct field and inject the dependency (often in `main`).
...
If you're writing a library/framework, source code of the Go SDK is a good place to look at. For applications composed of different modules, some people recommend [golang-standards/project-layout](https://github.com/golang-standards/project-layout). This repo does not follow that layout; see [../go.md](../go.md).

A simple check that design is testable: if you cannot write a unit test without standing up a database, mailer, or queue, the dependency boundary is probably wrong.
...
4. [Error handling in Go](https://go.dev/blog/error-handling-and-go)
5. [Errors are values](https://go.dev/blog/errors-are-values)
6. [Slice tricks](https://go.dev/wiki/SliceTricks)
7. [Table driven testing](https://go.dev/wiki/TableDrivenTests)
...
10. [Twelve Go Best Practices — Francesc Campoy Flores](https://go.dev/talks/2013/bestpractices.slide)
